...
//       const response = await axios.post(buildApiUrl('/api/auth/login'), { usn, password });
...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // const response = await axios.post(`${BASE_API}/auth/login`, { usn, password });
      const response = await axios.post(`http://localhost:7071/api/users/auth/login`, { usn, password });
