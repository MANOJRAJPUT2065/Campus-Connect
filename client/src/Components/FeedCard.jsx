...
const FeedCard = ({ postId, title, content, image, author, username, createdAt }) => {
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      await axios.post(`http://localhost:7071/likePost`, { postId, userEmail: userEmail });
      //http://localhost:7071
      setIsLiked(true);
  ...
  const handleUnlike = async () => {
    ...
    try {
      ...
      await axios.delete(`http://localhost:7071/likePost/toggle`, { data: { postId, userEmail: userEmail } });
      //http://localhost:7071
      setIsLiked(false);
      setLikes(prevLikes => prevLikes - 1);
    } catch (error) {
  ...
  const handleBookmark = async () => {
    ...
    try {
      ...
      const bookmarks = await axios.get(`http://localhost:7071/bookmark/check/${postId}?userEmail=${userEmail}`);
      //http://localhost:7071
      const bookmarkedPost = bookmarks.data.bookmarked;
      if (bookmarkedPost) {
        await axios.delete(`http://localhost:7071/bookmark/remove/${postId}/${userEmail}`);
        setIsBookmarked(false);
      } else {
        await axios.post(`http://localhost:7071/bookmark/add`, { postId, userEmail: userEmail });
  ...
  useEffect(() => {
    const checkLikeStatus = async () => {
      ...
      const userEmail = data.email;
      try {
        const likes = await axios.get(`http://localhost:7071/likePost/${postId}`);
    ...
    const fetchLikes = async () => {
      try {
        const response = await axios.get(`http://localhost:7071/likePost/${postId}`);
  ...
  const handleCommentSubmit = async () => {
    try {
      const response = await axios.post(`http://localhost:7071/comment/post`, {
        postId: postId,
        content: comment,
      }, {
        headers: {
  ...
  useEffect(() => {
    console.log("To get User Details from frontend");
    axios.get(`http://localhost:7071/auth/getUseretails`, {
      headers: {
