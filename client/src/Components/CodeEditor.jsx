...
const CodeEditor = React.memo(({ onClose }) => {
  ...
  // Bad words filter list (educational purposes - inappropriate language detection)
  const badWords = [
    'badword1', 'badword2', 'badword3', 'abusive', 'offensive',
  ...
  const checkContentSafety = (text) => {
    ...
    if (foundBadWords.length > 0) {
      setContentWarning(`⚠️ Inappropriate content detected: "${foundBadWords.join(', ')}" - Please remove before execution`);
      return false;
    }
    setContentWarning('');
    return true;
...
int main() {
  ...
  // 🚀 Execute code - Try RapidAPI first, fallback to local JS execution
  const executeCode = async (code, languageChoice, inputData = "") => {
    try {
      // Check for bad content first
      if (!checkContentSafety(code)) {
        return { success: false, output: "⚠️ Code contains inappropriate content. Please remove it." };
      }

      // For JavaScript, try local execution first (sandbox)
      if (languageChoice === 'javascript' || language === 'javascript') {
        try {
          return executeJavaScriptLocally(code, inputData);
        } catch (localError) {
          console.log('Local JS execution failed, trying RapidAPI...');
        }
      }

      // Try RapidAPI
      if (!RAPID_API_KEY) {
        console.warn('⚠️ RapidAPI key not configured. Using local execution for JS or mock output.');
        if (languageChoice === 'javascript' || language === 'javascript') {
          return executeJavaScriptLocally(code, inputData);
        }
        return { success: false, output: "⚠️ RapidAPI key not configured. Please set VITE_RAPID_API_KEY in .env.local" };
      }
      ...
      if (error.response?.status === 401) {
        return { success: false, output: "❌ API Authentication Failed: Invalid or missing RapidAPI key. Please check your .env.local file." };
      }
      if (error.response?.status === 429) {
        return { success: false, output: "⏱️ Rate limit exceeded. Please wait a moment and try again." };
      }
      if (error.code === 'ECONNABORTED') {
        return { success: false, output: "⏱️ Request timeout. The code compiler is taking too long. Try simpler code." };
      }
      if (!RAPID_API_KEY) {
        return { success: false, output: "⚠️ RapidAPI key not configured. Set VITE_RAPID_API_KEY in .env.local" };
      }
  ...
  // 🛡️ Local JavaScript execution (safe sandbox for JS)
  const executeJavaScriptLocally = (jsCode, inputData = "") => {
    try {
      ...
      const safeConsole = {
        ...
        info: (...args) => { output += 'ℹ️ ' + args.join(' ') + '\n'; },
      };

      // Create safe function with limited scope
      const func = new Function('console', 'input', jsCode);
      func(safeConsole, inputData);

      return { success: true, output: output || "Code executed successfully (no output)." };
    } catch (error) {
      return { success: false, output: `JavaScript Error:\n${error.message}\n\nLine: ${error.stack}` };
    }
  };
  ...
  const runCode = async () => {
    ...
    // Check for inappropriate content
    if (!checkContentSafety(code)) {
      toast.error("⚠️ Inappropriate content detected! Please remove it before running.");
      return;
    }
